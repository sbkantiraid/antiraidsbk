...
const Dashboard = () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          if (data.user) {
            ...
            // Filtrar servidores donde el usuario es admin y el bot está añadido
            const adminServers = data.guilds.filter(guild => {
              const permissions = parseInt(guild.permissions)
              const isAdmin = (permissions & 8) === 8 // 8 = ADMINISTRATOR
              return isAdmin
            })
  ...
  const handleSelectServer = (server) => {
    ...
    localStorage.setItem('selectedGuild', server.id)
    setPage('inicio')
  }
        ...
        <div className="max-w-2xl w-full">
          <div className="bg-zinc-900 border border-zinc-800 rounded-3xl p-8 shadow-2xl">
            ...
            <div className="flex items-center justify-between mb-8 bg-zinc-800/50 rounded-xl p-4">
              <div>
                <p className="text-white font-bold">@{user?.username || 'Usuario'}</p>
            ...
            {servers.length === 0 ? (
              <div className="bg-yellow-600/20 border border-yellow-600/50 rounded-2xl p-6 text-center">
                <p className="text-yellow-400 font-bold">No hay servidores disponibles</p>
                <p className="text-yellow-300/80 text-sm mt-2">
                  Necesitas ser administrador en un servidor y que el bot esté añadido.
                </p>
              </div>
            ) : (
              <div className="space-y-4">
                {servers.map((server) => (
                  ...
                  >
                    <div className="w-14 h-14 bg-indigo-600 rounded-full flex items-center justify-center text-2xl font-bold">
